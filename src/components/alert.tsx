...
export const useAlertStore = create<AlertState & AlertActions>()(
    (set, get) => ({
        ...initialAlertState,
        addAlert: (message, type = 'info', dismissInMs = 5000) => {
            const id = v4();
...
export const AlertItem = ({
    ...
    <div className='flex items-center justify-between overflow-hidden rounded-lg bg-white shadow-lg'>
        <div className='flex items-center gap-3'>
            ...
            <div className='w-full text-sm font-medium'>{alert.message}</div>
        </div>
        <button
            type='button'
...
export const AlertList = () => {
    ...
    return (
        <div
            className='fixed left-1/2 top-6 z-30 w-full max-w-xs -translate-x-1/2 space-y-2'
            role='alert'>
